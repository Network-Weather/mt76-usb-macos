...
class Mt7921u:
    ...
    def _vendor(self, req: int, req_type: int, value: int, index: int,
                data_or_len, timeout: int = VEND_TIMEOUT_MS):
        """__mt76u_vendor_request, including its retry loop."""
        last = None
        for _ in range(VEND_RETRIES):
            try:
                return self.dev.ctrl_transfer(req_type, req, value, index,
                                              data_or_len, timeout)
            except usb.core.USBError as exc:
                last = exc
                time.sleep(0.005)
        raise RuntimeError(
            f"vendor request req:{req:02x} off:{index:04x} failed: {last}")

    def rr(self, addr: int) -> int:
        """mt792xu_rr: 32-bit register read via MT_VEND_READ_EXT."""
        buf = self._vendor(MT_VEND_READ_EXT,
                           USB_DIR_IN | MT_USB_TYPE_VENDOR,
                           (addr >> 16) & 0xFFFF, addr & 0xFFFF, 4)
    ...
    def wr(self, addr: int, val: int) -> None:
        ...
        self._vendor(MT_VEND_WRITE_EXT,
    ...
    def uhw_rr(self, addr: int) -> int:
        ...
        buf = self._vendor(MT_VEND_DEV_MODE,
    ...
    def copy(self, offset: int, data: bytes, chunk: int = 512) -> None:
        ...
        while i < len(data):
            ...
            self._vendor(MT_VEND_WRITE_EXT,
                         USB_DIR_OUT | MT_USB_TYPE_VENDOR,
                         ((offset + i) >> 16) & 0xFFFF, (offset + i) & 0xFFFF,
                         data[i:i + n])
            i += n

    def power_on(self) -> None:
        """mt792xu_mcu_power_on."""
        self._vendor(MT_VEND_POWER_ON,
                     USB_DIR_OUT | MT_USB_TYPE_VENDOR, 0x0, 0x1, None)
        if not self.poll(MT_CONN_ON_MISC, MT_TOP_MISC2_FW_PWR_ON,
...
(PATCH_NOT_DL_SEM_FAIL, PATCH_IS_DL,
 PATCH_NOT_DL_SEM_SUCCESS, PATCH_REL_SEM_SUCCESS) = 0, 1, 2, 3
...
class Mt7921uMcu(Mt7921u):
    ...
    def _build_mcu_txd(self, total_len: int, cid: int, seq: int,
                       ext_cid: int = 0, set_query: int = MCU_Q_NA,
                       s2d: int = MCU_S2D_H2N) -> bytes:
        ...
        out += b"\x00" * 20  # rsv[5]
        assert len(out) == MCU_TXD_LEN, len(out)
        return out

    def mcu_send(self, cid: int, payload: bytes = b"", wait: bool = True,
                 timeout: int = 3000):
        ...
        if self.verbose:
            print(f"    mcu -> cid=0x{cid:02x} seq={seq} "
        ...
        while time.monotonic() < deadline:
            try:
                ...
                raise McuError(
                    ...
                    f"0x{ep:02x} after {discarded} frames ({exc})") from exc
            ...
            if self.verbose:
                print(f"    mcu <- {len(raw)}B seq={rseq} (want {seq}, "
                      f"{discarded} frames skipped)")
            if rseq == seq:
                return raw
            discarded += 1
        raise McuError(
            f"cid 0x{cid:02x}: no response matching seq {seq} "
            f"({discarded} frames skipped)")
    ...
    def init_download(self, addr: int, length: int, mode: int) -> None:
        ...
        cid = (MCU_CMD_PATCH_START_REQ if addr == 0x900000
               else MCU_CMD_TARGET_ADDRESS_LEN_REQ)
    ...
    def nic_power_ctrl(self, power_mode: int = 1) -> None:
        ...
        self.mcu_send(MCU_CMD_NIC_POWER_CTRL,
                      struct.pack("<BBBB", power_mode, 0, 0, 0), wait=False)
...
def parse_patch(blob: bytes) -> dict:
    ...
    d_patch_ver, subsys, feature, n_region, crc = struct.unpack_from(">IIIII", blob, 32)
    ...
    for i in range(n_region):
        ...
        sections.append({
            "type": sec_type, "offs": offs, "size": size,
            "addr": addr, "len": length,
            "sec_key_idx": sec_key_idx, "align_len": align_len,
        })
    return {
        "build_date": build_date, "platform": platform,
        "hw_sw_ver": hw_sw_ver, "patch_ver": patch_ver, "checksum": checksum,
        "subsys": subsys, "feature": feature, "n_region": n_region, "crc": crc,
        "sections": sections,
    }
...
def parse_ram(blob: bytes) -> dict:
    ...
    for i in range(n_region):
        ...
        decomp_crc, decomp_len, decomp_blk_sz = struct.unpack_from("<III", blob, base)
        addr, length = struct.unpack_from("<II", blob, base + 16)
        feature_set, rtype = struct.unpack_from("<BB", blob, base + 24)
        regions.append({
            "addr": addr, "len": length,
            "feature_set": feature_set, "type": rtype,
            "decomp_crc": decomp_crc, "decomp_len": decomp_len,
        })
    return {
        "chip_id": chip_id, "eco_code": eco_code, "n_region": n_region,
        "format_ver": format_ver, "format_flag": format_flag,
        "fw_ver": fw_ver, "build_date": build_date, "crc": crc,
        "regions": regions,
    }
...
class Mt7921uDevice(Mt7921uMcu):
    ...
    def dma_prefetch(self) -> None:
        for idx, cnt, base in ((0, 4, 0x080), (1, 4, 0x0C0), (2, 4, 0x100),
                               (3, 4, 0x140), (4, 4, 0x180),
                               (16, 4, 0x280), (17, 4, 0x2C0)):
            self.rmw(MT_UWFDMA0_TX_RING_EXT_CTRL(idx),
    ...
    def wfdma_init(self) -> None:
        ...
        self.set_bits(MT_UWFDMA0_GLO_CFG,
        ...
        self.rmw(MT_DMASHDL_PKT_MAX_SIZE,
    ...
    def dma_init(self, resume: bool = False) -> None:
        ...
        self.set_bits(MT_UDMA_WLCFG_0,
                      MT_WL_RX_EN | MT_WL_TX_EN | MT_WL_RX_MPSZ_PAD0 | MT_TICK_1US_EN)
        self.rmw(MT_UDMA_WLCFG_1, MT_WL_TX_TMOUT_LMT,
    ...
    def load_patch(self, blob: bytes, log=print) -> None:
        ...
        try:
            p = parse_patch(blob)
            log(f"  patch {p['build_date']} hw/sw 0x{p['hw_sw_ver']:08x}, "
                f"{p['n_region']} region(s)")
            for i, sec in enumerate(p["sections"]):
                ...
                log(f"  section {i}: addr=0x{sec['addr']:08x} "
                    f"len={sec['len']:,} mode=0x{mode:08x}")
    ...
    def load_ram(self, blob: bytes, log=print) -> None:
        ...
        r = parse_ram(blob)
        log(f"  ram fw {r['fw_ver']} built {r['build_date']}, "
            f"{r['n_region']} regions")
        override = 0
        option = 0
        offset = 0
        for i, rg in enumerate(r["regions"]):
            ...
            if rg["feature_set"] & FW_FEATURE_NON_DL:
                ...
                offset += rg["len"]
                continue
            log(f"  region {i}: addr=0x{rg['addr']:08x} len={rg['len']:,} "
                f"mode=0x{mode:08x}")
    ...
    def bringup(self, patch_blob: bytes, ram_blob: bytes, log=print) -> None:
        ...
        if not self.poll(MT_CONN_ON_MISC, MT_TOP_MISC_FW_STATE,
        ...
        if not self.poll(MT_CONN_ON_MISC, MT_TOP_MISC2_FW_N9_RDY,
                         MT_TOP_MISC2_FW_N9_RDY, 1500):
            raise RuntimeError(
                f"timeout waiting for N9_RDY "
                f"(MT_CONN_ON_MISC = 0x{self.rr(MT_CONN_ON_MISC):08x})")
...
def _wait_udma_idle(self) -> None:
    ...
    if not self.poll(MT_UDMA_WLCFG_0, MT_WL_RX_BUSY | MT_WL_TX_BUSY, 0,
...
def _wfsys_reset(self) -> None:
    ...
    self.uhw_wr(MT_CBTOP_RGU_WF_SUBSYS_RST,
                val | MT_CBTOP_RGU_WF_SUBSYS_RST_WF_WHOLE_PATH)
    time.sleep(0.001)
    val = self.uhw_rr(MT_CBTOP_RGU_WF_SUBSYS_RST)
    self.uhw_wr(MT_CBTOP_RGU_WF_SUBSYS_RST,
                val & ~MT_CBTOP_RGU_WF_SUBSYS_RST_WF_WHOLE_PATH & 0xFFFFFFFF)
...
NIC_CAP_NAMES = {
    0x00: "TX_RESOURCE", 0x01: "TX_EFUSE_ADDR", 0x02: "COEX",
    0x03: "SINGLE_SKU", 0x04: "CSUM_OFFLOAD", 0x05: "HW_VER",
    0x06: "SW_VER", 0x07: "MAC_ADDR", 0x08: "PHY", 0x09: "MAC",
    0x0A: "FRAME_BUF", 0x0B: "BEAM_FORM", 0x0C: "LOCATION",
    0x0D: "MUMIMO", 0x0E: "BUFFER_MODE_INFO", 0x14: "HW_ADIE_VERSION",
    0x16: "ANTSWP", 0x17: "WFDMA_REALLOC", 0x18: "6G",
    0x20: "CHIP_CAP", 0x22: "EML_CAP",
}


def _mcu_cmd_word(self, cmd: int, payload: bytes = b"", wait: bool = True,
                  timeout: int = 3000):
    ...
    seq = self._next_seq()
    total = MCU_TXD_LEN + len(payload)
    body = self._build_mcu_txd(total, cid, seq, ext_cid=ext_cid,
                               set_query=set_query, s2d=s2d) + payload
    frame = struct.pack("<I", len(body) & 0xFFFF) + body
    frame += b"\x00" * (((len(frame) + 3) & ~3) + 4 - len(frame))

    if self.verbose:
        print(f"    mcu -> cmd=0x{cmd:06x} cid=0x{cid:02x} ext=0x{ext_cid:02x} "
...
def _set_chan_info(self, control_ch: int, center_ch: int, bw: int,
                   band: int, antenna_mask: int = 0x3,
    ...
    """
    rx_streams = bin(antenna_mask).count("1") if cmd_ext == MCU_EXT_CMD_CHANNEL_SWITCH \
        else antenna_mask
    req = struct.pack(
        "<BBBBBBBB",
        control_ch & 0xFF,
        center_ch & 0xFF,
        bw & 0xFF,
        bin(antenna_mask).count("1") & 0xFF,   # tx_streams_num
        rx_streams & 0xFF,
        CH_SWITCH_NORMAL,
        band_idx & 0xFF,
        0,                                      # center_ch2
    ...
    req += b"\x00" * 57                              # rsv1
    assert len(req) == 76, len(req)   # 19 bytes of fields + rsv1[57]
...
MONITOR_FILTER = (MT7921_FILTER_ENABLE | MT7921_FILTER_FCSFAIL
...
def _set_rxfilter(self, fif: int, bit_op: int = 0, bit_map: int = 0) -> None:
    ...
    data += b"\x00" * 51                    # pad
    assert len(data) == 68, len(data)   # 17 bytes of fields + pad[51]
...
MONITOR_DROP_CLEAR = (
    MT_WF_RFCR_DROP_STBC_MULTI | MT_WF_RFCR_DROP_VERSION
...
def _build_uni_txd(self, total_len: int, cid: int, seq: int) -> bytes:
    ...
    out += b"\x00" * 4
    assert len(out) == MCU_UNI_TXD_LEN, len(out)
    return out


def _mcu_uni(self, cid: int, payload: bytes, wait: bool = True,
             timeout: int = 3000):
    seq = self._next_seq()
...
def _config_sniffer(self, control_ch: int, center_ch: int, band_name: str,
                    bw: int = SNIFFER_BW_20, center_ch2: int = 0,
                    drop_err: int = 1, band_idx: int = 0) -> None:
...
def build_probe_request(src_mac: bytes, ssid: bytes = b"",
                        seq: int = 0) -> bytes:
...
def _build_txwi(self, frame: bytes, seq: int = 0, pid: int = 0) -> bytes:
    ...
    txwi[0] = (((len(frame) + MT_SDIO_TXD_SIZE) & MT_TXD0_TX_BYTES_M)
    ...
    txwi[8] = ((fc_type << MT_TXD8_L_TYPE_SHIFT)
               | (fc_stype << MT_TXD8_L_SUB_TYPE_SHIFT))
...
def _read_efuse(self, offset: int):
    ...
    rxd = self.mcu_cmd_word(
        MCU_EXT_CMD(MCU_EXT_CMD_EFUSE_ACCESS) | MCU_CMD_FIELD_QUERY, req)
    body = rxd[MCU_RXD_LEN:]
    if len(body) < 8 + MT7921_EEPROM_BLOCK_SIZE:
        return None, None
    addr, valid = struct.unpack_from("<II", body, 0)
