...
- [ ] **1. Count aggregated airtime once, not per subframe.** `scripts/mib_survey.py` sums
  ...
  *Done when:* a test drives the dwell loop with a synthetic A-MPDU and shows the summed
  airtime matching the aggregate rather than the per-frame total.
- [ ] **2. Measure the counter over the interval the denominator describes.** The offset-11
  ...
  *Done when:* a test asserts the dwell window encloses the frame loop, and `busy_fraction`
  cannot exceed 1 for that reason.
- [ ] **3. Stop counting ordinary receive timeouts as transport errors.**
  ...
  real transport error and only the second is counted.
- [ ] **4. Re-measure, then rewrite the numbers.** Every occupancy figure in
