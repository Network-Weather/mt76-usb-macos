...
CH_24 = [1, 6, 11]
CH_5 = [36, 40, 44, 48, 52, 56, 60, 64, 100, 104, 108, 112, 116,
...
def main() -> int:
    ...
    with m.Mt7921uDevice() as dev:
        ...
        print(f"scanning {len(plan)} channels, {DWELL:g}s each "
              f"(~{len(plan) * DWELL:.0f}s)\n")

        for band, ch in plan:
            dev.set_chan_info(control_ch=ch, center_ch=ch,
                              bw=m.CMD_CBW_20MHZ, band=CHAN_BAND[band])
            dev.config_sniffer(control_ch=ch, center_ch=ch, band_name=band,
                               bw=m.SNIFFER_BW_20)
            time.sleep(0.05)
            kinds = Counter()
            n = 0
            ...
            while time.monotonic() < deadline:
                ...
                if p["kind"] in ("Beacon", "ProbeResp") and p.get("addr3"):
                    k = p["addr3"]
                    e = bss.setdefault(k, {"ssid": p.get("ssid", "?"),
                                           "band": band, "ch": ch, "n": 0,
                                           "rssi": None, "load": None})
                    e["n"] += 1
                    if d.get("rssi") is not None and (
                            e["rssi"] is None or d["rssi"] > e["rssi"]):
                        e["rssi"] = d["rssi"]
                    if p.get("bss_load"):
                        e["load"] = p["bss_load"]
            bc = kinds["Beacon"] + kinds["ProbeResp"]
            mark = "*" if bc else " "
            print(f" {mark} {band:>6} ch {ch:>3}: {n:>5} xfers, "
                  f"{bc:>4} beacons/probe-resp")

    print()
    by_band = Counter(v["band"] for v in bss.values())
    print(f"{len(bss)} BSSIDs: " + ", ".join(f"{b} {c}" for b, c in by_band.items()))
    print()
    print(f"  {'BSSID':<19} {'band':>6} {'ch':>4} {'RSSI':>5} {'util':>5} "
          f"{'sta':>4}  SSID")
    for k, v in sorted(bss.items(),
                       key=lambda kv: (kv[1]["band"], kv[1]["ch"],
                                       -(kv[1]["rssi"] or -999))):
        load = v["load"] or {}
        util = f"{load['channel_util_pct']}%" if load else ""
        sta = load.get("stations", "")
        r = str(v["rssi"]) if v["rssi"] is not None else "?"
        print(f"  {k:<19} {v['band']:>6} {v['ch']:>4} {r:>5} {util:>5} "
              f"{str(sta):>4}  {v['ssid']}")
    return 0
